...
public:
  ...
  inline std::uint32_t getNumberOfFrames()
  {
    return m_Frames.size();
  }
  inline unsigned char* getFramePtr(const std::uint32_t& i)
  {
    return m_Frames[i];
  }
  inline std::uint32_t getFrameAsicHeader(const std::uint32_t& i)
  {
    return (&m_DIF[i])[DU::FRAME_ASIC_HEADER_SHIFT];
  }
  inline std::uint32_t getFrameBCID(uint32_t i)
  {
  ...
  inline std::uint32_t getFrameTimeToTrigger(const std::uint32_t& i)
  {
    return getBCID()-getFrameBCID(i);
  }
  inline bool getFrameLevel(const std::uint32_t& i,const std::uint32_t& ipad,const std::uint32_t& ilevel)
  {
    return ((m_Frames[i][DU::FRAME_DATA_SHIFT+((3-ipad/16)*4+(ipad%16)/4)]>>(7-(((ipad%16)%4)*2+ilevel))) & 0x1);
  }
  void dumpDIFInfo();
  //Addition by GG
  inline std::uint32_t getDIFid() 
  {
    return getID()&0xFF;
  }
  inline std::uint32_t getASICid(const std::uint32_t& i) 
  {
    return getFrameAsicHeader(i)&0xFF;
  }
  inline std::uint32_t getThresholdStatus(const std::uint32_t& i,const std::uint32_t& ipad) 
  {
