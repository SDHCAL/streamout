...
void DIFSlowControl::FillAsicHR1(const std::bitset<72*8> &bs)
{
  ...
  {
    int gain{0};
    for (int j=0;j<6;j++) if (bs[176+i*6+j]!=0) gain+= (1<<j);
...
void DIFSlowControl::FillAsicHR2(const std::bitset<109*8>&bs)
{
  ...
  for(int i=0;i<64;i++)
  {
    ...
    for(int j=0;j<8;j++) if (bs[64+i*8+j]!=0) gain +=(1<<j);
    mAsic["Channel_"+std::to_string(i)+"_"+"Gain"]=gain;
    for (int j=0;j<3;j++) if (bs[8*77+2+i*3+j]!=0) mask+=(1<<j);
  ...
  {
    int B=0;
    for (int j=0;j<10;j++) if (bs[8*102+2+i*10+j]!=0) B+=(1<<j);
