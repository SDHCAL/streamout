...
void BufferLooperCounter::printCounter(const std::string& description, const std::map<int, int>& m, const std::ios_base::fmtflags& base)
{
  ...
  for(std::map<int, int>::const_iterator it = m.begin(); it != m.end(); it++)
  {
    ...
    switch(base)
    {
      case std::ios_base::dec: out += to_dec(static_cast<std::uint32_t>(it->first)); break;
      case std::ios_base::hex: out += to_hex(static_cast<std::uint32_t>(it->first)); break;
      case std::ios_base::oct: out += to_oct(static_cast<std::uint32_t>(it->first)); break;
      default: out += to_dec(static_cast<std::uint32_t>(it->first)); break;
    }
