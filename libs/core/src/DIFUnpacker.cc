...
unsigned long long DIFUnpacker::GrayToBin(const unsigned long long& n)
{
...
unsigned long long DIFUnpacker::getAbsoluteBCID(const unsigned char* cb, const std::uint32_t& idx)
{
  ...
  unsigned long long LBC = ((cb[idx + DU::ABCID_SHIFT] << 16) | (cb[idx + DU::ABCID_SHIFT + 1] << 8) | (cb[idx + DU::ABCID_SHIFT + 2])) * Shift +
                           ((cb[idx + DU::ABCID_SHIFT + 3] << 16) | (cb[idx + DU::ABCID_SHIFT + 4] << 8) | (cb[idx + DU::ABCID_SHIFT + 5]));
  return LBC;
}
