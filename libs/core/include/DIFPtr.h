...
{
public:
  DIFPtr(unsigned char* p, const std::uint32_t& max_size);
  inline unsigned char*               getPtr() { return theDIF_; }
  inline std::uint32_t                getGetFramePtrReturn() { return theGetFramePtrReturn_; }
  ...
  inline std::uint32_t                getID() { return DIFUnpacker::getID(theDIF_); }
  inline std::uint32_t                getDTC() { return DIFUnpacker::getDTC(theDIF_); }
  inline std::uint32_t                getGTC() { return DIFUnpacker::getGTC(theDIF_); }
  inline std::uint64_t                getAbsoluteBCID() { return DIFUnpacker::getAbsoluteBCID(theDIF_); }
  inline std::uint32_t                getBCID() { return DIFUnpacker::getBCID(theDIF_); }
  inline std::uint32_t                getLines() { return DIFUnpacker::getLines(theDIF_); }
  inline bool                         hasLine(uint32_t line) { return DIFUnpacker::hasLine(line, theDIF_); }
  inline std::uint32_t                getTASU1() { return DIFUnpacker::getTASU1(theDIF_); }
  inline std::uint32_t                getTASU2() { return DIFUnpacker::getTASU2(theDIF_); }
  inline std::uint32_t                getTDIF() { return DIFUnpacker::getTDIF(theDIF_); }
  ...
  inline std::uint32_t                getNumberOfFrames() { return theFrames_.size(); }
  inline unsigned char*               getFramePtr(uint32_t i) { return theFrames_[i]; }
  inline std::uint32_t                getFrameAsicHeader(uint32_t i) { return DIFUnpacker::getFrameAsicHeader(theFrames_[i]); }
...
private:
  ...
  std::uint32_t               theGetFramePtrReturn_;
